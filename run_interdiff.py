...
def run_interdiff(repo, backport_sha, upstream_sha, interdiff_path):
    ...
    backport_patch = subprocess.run(
        ['git', '-C', repo, 'format-patch', '-1', '--stdout', backport_sha],
        text=True,
        capture_output=True,
        check=False
    )
    if backport_patch.returncode != 0:
        return False, f"Failed to generate patch for backport commit: {backport_patch.stderr}"

    # Generate format-patch for upstream commit
    upstream_patch = subprocess.run(
        ['git', '-C', repo, 'format-patch', '-1', '--stdout', upstream_sha],
        text=True,
        capture_output=True,
        check=False
    )
    if upstream_patch.returncode != 0:
        return False, f"Failed to generate patch for upstream commit: {upstream_patch.stderr}"

    # Run interdiff
    interdiff_result = subprocess.run(
        [interdiff_path],
        input=backport_patch.stdout + '\n---DELIMITER---\n' + upstream_patch.stdout,
        text=True,
        capture_output=True,
        check=False
    )

    # Use process substitution alternative: write to temp files
    import tempfile
    with tempfile.NamedTemporaryFile(mode='w', suffix='.patch', delete=False) as bp:
        bp.write(backport_patch.stdout)
        bp_path = bp.name

    with tempfile.NamedTemporaryFile(mode='w', suffix='.patch', delete=False) as up:
        up.write(upstream_patch.stdout)
        up_path = up.name

    try:
        interdiff_result = subprocess.run(
            [interdiff_path, bp_path, up_path],
            text=True,
            capture_output=True,
            check=False
        )
